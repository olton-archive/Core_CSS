/*!
 * Restyle CSS v2.0.0 (http://pimenov.com.ua)
...
// Source: js/global.js
if (window.CALENDAR_WEEK_START === undefined) window.CALENDAR_WEEK_START = 0;
if (window.CALENDAR_LOCALE === undefined) window.CALENDAR_LOCALE = "ru";
if (window.RESTYLE_DEBUG === undefined) window.RESTYLE_DEBUG = false;
    ...
    return function (date, mask, utc) {
        ...
        var _ = utc ? "getUTC" : "get",
            ...
            flags = {
                d: d,
                dd: pad(d),
                ddd: CALENDAR_LOCALES[locale].days[D],
                dddd: CALENDAR_LOCALES[locale].days[D + 7],
                m: m + 1,
                mm: pad(m + 1),
                mmm: CALENDAR_LOCALES[locale].months[m],
                mmmm: CALENDAR_LOCALES[locale].months[m + 12],
                yy: String(y).slice(2),
                yyyy: y,
                h: H % 12 || 12,
                hh: pad(H % 12 || 12),
...
var Restyle = {
    ...
    init: function(){
        ...
        Restyle.initHotkeys(hotkeys);
        Restyle.initWidgets(widgets);
...
$.widget("restyle.accordion", {
    ...
    init: function(){
        ...
        if (!element.hasClass('accordion')) { element.addClass('accordion'); }

        element.on('click', '.accordion-frame-heading', function(e){
    ...
    _closeAllFrames: function(){
        var that = this;
        var frames = this.element.children('.accordion-frame.active');
    ...
    _openFrame: function(frame){
        var o = this.options;
        var content = frame.children('.accordion-frame-content');
...
// Source: js/widgets/affix.js
$.widget("restyle.affix", {
...
$.widget("restyle.carousel", {
    ...
    _create: function(){
        var carousel, that = this, o = this.options,
            element = carousel = this.element,
            controls = carousel.find('.carousel-switch');

        if (!element.hasClass('carousel')) { element.addClass('carousel'); }
        ...
        this._slides = carousel.find('.carousel-frame');

        if (this._slides.length <= 1) return;

        if (!carousel.find('.carousel-bullets').length) {
            this._markers(that);
        }

        if (this.options.controls && this._slides.length > 1) {
            carousel.find('.carousel-switch-prev').on('click', function(){
                that._slideTo('prior');
            });
            carousel.find('.carousel-switch-next').on('click', function(){
                that._slideTo('next');
            });
        } else {
            controls.hide();
...
$.widget("restyle.hint", {
    ...
    createHint: function(){
        var that = this, element = this.element,
            ...
            o = this.options, hint_title, hint_text;
        ...
        hint_title = hint.length > 1 ? hint[0] : false;
        hint_text = hint.length > 1 ? hint[1] : hint[0];
...
// Source: js/widgets/infopanel.js
$.widget("restyle.infopanel", {
    ...
    _create: function(){
        ...
        if (!element.hasClass('info-panel')) { element.addClass('info-panel'); }

        if (element.hasClass("collapsible")) {
            var toggle = element.children(".info-panel-title");
            var content = element.children(".info-panel-content");
...
// Source: js/widgets/inspector.js
/**
 * Object structure
 *
 * object = {
...
var RESTYLE_INSPECTOR;

var RESTYLE_INSPECTOR_OBJECT_TYPE = {};

$.Inspector = function(params){
    if (!$.Inspector.opened) {
    ...
    params = $.extend({
        ...
        title: 'Properties',
        propertiesType: 'css',
        properties: RESTYLE_INSPECTOR_OBJECT_TYPE.css,
        objects: {}
    }, params);

    var _target, _inspector, _caption, _objects, _objects_list, _properties, _property;

    _target = params.parent || 'body';

    // Create inspector
    _inspector = $("<div/>").addClass("inspector").css({
        position: 'absolute'
    });
    if (params.draggable) {
        _inspector.addClass("draggable");
    }

    // Create caption
    _caption = $("<div/>").addClass("inspector-caption").appendTo(_inspector);
    $("<div/>").addClass("inspector-caption-title").html(params.title).appendTo(_caption);

    // Create objects
    _objects = $("<div/>").addClass("inspector-objects").appendTo(_inspector);
    _objects_list = $("<select/>").appendTo(_objects);

    // Create properties
    _properties = $("<div/>").addClass("inspector-properties").appendTo(_inspector);


    _inspector.appendTo($(_target)).show();

    RESTYLE_INSPECTOR = _inspector;

    return RESTYLE_INSPECTOR;
};

$.Inspector.inspect = function(object){
...
// Source: js/widgets/popover.js
$.widget("restyle.popover", {
...
$.widget("restyle.tabcontrol", {
    ...
    _create: function(){
        ...
        element.on("click", ".tab-control-tabs a", function(e){
            e.preventDefault();
            e.stopPropagation();
            ...
            element.find(".d-menu").hide();

            element.children(".tab-control-tabs").find("a").each(function(){
                $(this).parent("li").removeClass("active");
            });
            $(element.children(".tab-control-frames")).children(".tab-control-frame").hide();
    ...
    init: function(){
        var that = this, element = this.element,
            tabs = $(element.children(".tab-control-tabs")).find("li"),
            frames = $(element.children(".tab-control-frames")).children(".tab-control-frame");

        if (!element.hasClass('tab-control')) { element.addClass('tab-control'); }
        
        tabs.each(function(){
            if ($(this).hasClass("active")) {
                var current_frame = $($($(this).find("a")).attr("href"));
                frames.hide();
                current_frame.show();
                that._trigger('change', null, current_frame);
                return true;
...
// Source: js/widgets/toolbars.js
$.widget("restyle.toolbar", {
    ...
    _create: function(){
        ...
        var _check_sections = element.find(".toolbar-group-check");
        var _radio_sections = element.find(".toolbar-group-radio");

        $.each(_check_sections, function(){
            var section = $(this), this_section = this;
            section.find("button").on("click", function(){
                $(this).toggleClass("checked");
            });
        });
        $.each(_radio_sections, function(){
            var section = $(this), this_section = this;
            section.find("button").on("click", function(){
                section.find("button").removeClass("checked");
